...
{
	$listeligne=$db->query("SELECT * FROM ligne where id_commande=1");
}
else if(isset($_POST['tria']))
{
	$listeligne=$db->query("SELECT * FROM ligne ORDER BY ligne.`id_commande` DESC");
}
else 
$listeligne=$r->afficherlignes();

?>
<form method="POST">
					  	<button width="50%" type="submit" name="tri" class="submit check_out"></button> 
					  	<button width="50%" type="submit" name="tria" class="submit check_out"></button> 
					  </form>
...
foreach($listeligne as $row){
	...
	<td><a href='../core/supprimerligne.php?id=<?php echo$row['id_commande'];?>'>SUPPRIMER</a>
	<td><a href='modifierligne.php?id_commande=<?php echo$row['id_commande'];?>'>MODIFIER</a>
	</tr>
	<?PHP
}
?>
